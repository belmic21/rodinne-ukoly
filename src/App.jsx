...
/* ═══════════════════════════════════════════════════════
   RODINNÉ ÚKOLY — Produkční verze pro Supabase
   ═══════════════════════════════════════════════════════ */
...
async function apiCreateUser(u) {
  const { error } = await supabase.from("users").insert({ name: u.name, pin: u.pin, is_admin: u.admin });
...
async function apiCreateTask(t) {
  const { error } = await supabase.from("tasks").insert(taskToDb(t));
  if (error) console.error(error);
}

async function apiUpdateTask(t) {
  const { error } = await supabase.from("tasks").update(taskToDb(t)).eq("id", t.id);
  if (error) console.error(error);
}

async function apiUpdateTasks(ts) {
  for (const t of ts) await apiUpdateTask(t);
}
...
function Setup({ onDone }) {
  ...
  const create = async () => {
    ...
    setBusy(true);
    await onDone({ name: name.trim(), pin, admin: true });
  };
  return (
    <div style={{ minHeight: "100vh", background: th.bg, fontFamily: F, color: th.text, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
      ...
      <div style={{ width: "300px", textAlign: "center", animation: "fIn 0.4s" }}>
        ...
        <input placeholder="Tvoje jméno" value={name} onChange={e => setName(e.target.value)}
          style={{ ...inpS(th), padding: "12px", fontSize: "15px", marginBottom: "10px", textAlign: "center" }} />
        <input placeholder="4místný PIN" value={pin} onChange={e => { if (/^\d{0,4}$/.test(e.target.value)) setPin(e.target.value); }} type="tel" inputMode="numeric" maxLength={4}
          style={{ ...inpS(th), padding: "12px", fontSize: "20px", marginBottom: "16px", textAlign: "center", letterSpacing: "8px" }} />
        <button onClick={create} disabled={!name.trim() || pin.length !== 4 || busy}
          style={{ ...btnS(th), width: "100%", padding: "12px", background: name.trim() && pin.length === 4 ? th.accent : th.btnBg, color: "#fff", fontSize: "14px", opacity: !name.trim() || pin.length !== 4 || busy ? 0.4 : 1 }}>
...
function Login({ users, onLogin, theme }) {
  ...
  const tryLogin = () => {
    ...
    if (u && u.pin === pin) { setErr(false); onLogin(u); }
      ...
      <div style={{ width: "320px", textAlign: "center", animation: "fIn 0.4s" }}>
        ...
        {!sel ? (
          ...
          <div style={{ animation: "sUp 0.2s" }}>
            ...
            <input placeholder="PIN" value={pin} onChange={e => { if (/^\d{0,4}$/.test(e.target.value)) { setPin(e.target.value); setErr(false); } }} onKeyDown={e => e.key === "Enter" && pin.length === 4 && tryLogin()} type="tel" inputMode="numeric" maxLength={4} autoFocus
              style={{ ...inpS(th), padding: "12px", fontSize: "22px", marginBottom: "8px", textAlign: "center", letterSpacing: "10px", borderColor: err ? th.red : th.inputBorder }} />
            {err && <div style={{ fontSize: "12px", color: th.red, marginBottom: "8px" }}>Špatný PIN</div>}
            <button onClick={tryLogin} disabled={pin.length !== 4} style={{ ...btnS(th), width: "100%", padding: "11px", background: pin.length === 4 ? th.accent : th.btnBg, color: "#fff", fontSize: "14px", opacity: pin.length === 4 ? 1 : 0.4, marginBottom: "8px" }}>
              Přihlásit
            </button>
  ...
  return (
    <div style={{ ...cardS(th), padding: "16px", marginBottom: "14px", animation: "sUp 0.2s" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
        ...
        <button onClick={onClose} style={{ background: "none", border: "none", color: th.textSub, cursor: "pointer", fontSize: "16px" }}>×</button>
      </div>
      {users.map(u => (
      ...
      <div style={{ display: "flex", gap: "6px", marginTop: "12px" }}>
        ...
        <button onClick={() => { if (name.trim() && pin.length === 4 && !users.find(u => u.name === name.trim())) { onAdd({ name: name.trim(), pin, admin: false }); setName(""); setPin(""); } }} style={{ ...btnS(th), padding: "8px 14px", background: th.accent, color: "#fff", fontSize: "12px" }}>+</button>
      </div>
    </div>
  );
}
...
function Checklist({ items = [], onChange, user, th }) {
  ...
  return (
    <div style={{ marginTop: "8px" }} onClick={e => e.stopPropagation()}>
      ...
      {items.map(it => (
        <div key={it.id} style={{ display: "flex", alignItems: "center", gap: "8px", padding: "5px 8px", borderRadius: "6px", background: it.done ? `${th.green}08` : th.inputBg, border: `1px solid ${it.done ? th.green + "15" : th.inputBorder}`, marginBottom: "3px" }}>
          <button onClick={() => toggle(it.id)} style={{ width: "17px", height: "17px", minWidth: "17px", borderRadius: "4px", border: `1.5px solid ${it.done ? th.green : th.textDim}`, background: it.done ? th.green : "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: "9px", fontWeight: 800 }}>
            {it.done && "✓"}
          </button>
      ...
      <div style={{ display: "flex", gap: "5px", marginTop: "5px" }}>
        <input placeholder="Přidat položku..." value={val} onChange={e => setVal(e.target.value)} onKeyDown={e => e.key === "Enter" && add()} style={{ ...inpS(th), fontSize: "12px", padding: "7px 10px", flex: 1 }} />
        <button onClick={add} style={{ ...btnS(th), padding: "7px 12px", background: th.btnBg, color: th.textSub, fontSize: "12px", border: `1px solid ${th.cardBorder}` }}>+</button>
      </div>
    </div>
  );
}
...
function Images({ images = [], onChange, th }) {
  ...
  const add = e => {
    ...
    if (f.size > 1048576) { alert("Max 1 MB pro obrázek (kvůli šetření prostoru)"); return; }
    const r = new FileReader();
    r.onload = () => { onChange([...images, { id: uid(), data: r.result }]); };
        ...
        <div style={{ display: "flex", gap: "5px", flexWrap: "wrap", marginBottom: "6px" }}>
          {images.map(img => (
            <div key={img.id} style={{ position: "relative", width: "56px", height: "56px", borderRadius: "6px", overflow: "hidden", border: `1px solid ${th.cardBorder}` }}>
              ...
              <button onClick={() => onChange(images.filter(i => i.id !== img.id))} style={{ position: "absolute", top: "1px", right: "1px", background: "rgba(0,0,0,0.65)", border: "none", color: "#fff", borderRadius: "50%", width: "15px", height: "15px", fontSize: "9px", cursor: "pointer", lineHeight: "15px", textAlign: "center", padding: 0 }}>×</button>
            </div>
          ))}
        </div>
      )}
      <input ref={ref} type="file" accept="image/*" capture="environment" onChange={add} style={{ display: "none" }} />
      <button onClick={() => ref.current?.click()} style={{ ...btnS(th), background: th.inputBg, border: `1px solid ${th.inputBorder}`, color: th.textSub, padding: "5px 10px", fontSize: "11px" }}>📷 Přidat fotku</button>
    </div>
  );
}
...
function TaskCard({ task: t, user, users, onStatus, onSeen, onUpdate, th }) {
  ...
  return (
    <div onClick={handleOpen} style={{
      ...cardS(th), borderLeft: `3px solid ${bc}`, padding: "11px 13px",
      opacity: isDn ? 0.4 : 1, cursor: "pointer", position: "relative",
      ...
      ...(fg && !isDn ? { background: `linear-gradient(135deg,${th.card} 80%,${th.purple}08 100%)` } : {}),
    }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: "10px" }}>
        <div style={{
          width: "20px", height: "20px", minWidth: "20px", marginTop: "1px", borderRadius: "5px",
          display: "flex", alignItems: "center", justifyContent: "center",
          fontSize: "10px", fontWeight: 700,
          background: isDn ? (t.status === "cancelled" ? `${th.priLow}15` : `${th.green}18`) : inPr ? `${th.yellow}12` : th.inputBg,
          color: isDn ? (t.status === "cancelled" ? th.priLow : th.green) : inPr ? th.yellow : th.textMid,
          border: `1.5px solid ${isDn ? (t.status === "cancelled" ? th.priLow + "30" : th.green + "35") : inPr ? th.yellow + "25" : th.inputBorder}`,
        }}>
          {t.status === "done" ? "✓" : t.status === "cancelled" ? "⊘" : inPr ? "◐" : "○"}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: "13.5px", fontWeight: 500, color: isDn ? th.textSub : th.text, textDecoration: t.status === "done" ? "line-through" : "none", lineHeight: 1.4, wordBreak: "break-word" }}>
            {t.title}
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: "6px", marginTop: "4px", flexWrap: "wrap" }}>
            <span style={{ fontSize: "10px", fontWeight: 700, color: pc[p.id], textTransform: "uppercase" }}>● {p.label}</span>
            {t.category && <span style={{ fontSize: "10px", color: th.textMid }}>{gc(t.category).icon}</span>}
            {clTotal > 0 && (
              <span style={{ display: "inline-flex", alignItems: "center", gap: "3px" }}>
                <span style={{ width: "32px", height: "3px", borderRadius: "2px", background: th.inputBorder, overflow: "hidden", display: "inline-block" }}>
                  <span style={{ display: "block", height: "100%", width: `${clTotal > 0 ? clDone / clTotal * 100 : 0}%`, background: clDone === clTotal ? th.green : th.accent, borderRadius: "2px", transition: "width 0.3s" }} />
                </span>
                <span style={{ fontSize: "10px", color: th.textSub, fontWeight: 600 }}>{clDone}/{clTotal}</span>
              </span>
            )}
            {t.assignTo === "both" && <span style={{ display: "inline-flex", gap: "2px", alignItems: "center" }}>
              {users.map(u => {
                const d = t.doneBy?.includes(u.name);
                return <span key={u.name} title={u.name} style={{ width: 7, height: 7, borderRadius: "50%", background: d ? th.green : th.inputBorder, border: `1px solid ${d ? th.green : th.textDim}` }} />;
              })}
            </span>}
      ...
      {open && (
        <div style={{ marginTop: "10px", paddingLeft: "30px", animation: "fIn 0.12s" }} onClick={e => e.stopPropagation()}>
          {t.note && <div style={{ fontSize: "12.5px", color: th.textSub, lineHeight: 1.5, padding: "7px 10px", background: th.inputBg, borderRadius: "6px", marginBottom: "8px", borderLeft: `2px solid ${th.inputBorder}` }}>{t.note}</div>}
          {(clTotal > 0 || (!isDn && t.type === "complex")) && (
            <Checklist items={t.checklist || []} user={user.name} th={th} onChange={cl => onUpdate(t.id, { checklist: cl })} />
          )}
          {(t.images?.length > 0 || !isDn) && (
            <Images images={t.images || []} th={th} onChange={imgs => onUpdate(t.id, { images: imgs })} />
          )}
...
function ABtn({ l, a, th, sub, style: sx }) {
  return <button onClick={a} style={{ ...btnS(th), padding: "5px 10px", fontSize: "11px", background: sub ? "transparent" : `${th.green}12`, color: sub ? th.textSub : th.green, border: `1px solid ${sub ? th.cardBorder : th.green + "25"}`, ...sx }}>{l}</button>;
}

/* ═══════════════════════════════════════════════════════
   ADD FORM + WEEKLY SUMMARY + SNACKBAR
   ═══════════════════════════════════════════════════════ */
function AddForm({ user, users, onAdd, th }) {
  ...
  const submit = () => {
    ...
    const to = assign === "self" ? [user.name] :
      assign === "person" ? [assignTarget || others[0]?.name] :
      assign === "both" ? users.map(u => u.name) : [user.name];
    onAdd({
  ...
  if (!op) return (
    <button onClick={() => { setOp(true); setTimeout(() => ref.current?.focus(), 80); }} style={{ ...cardS(th), width: "100%", padding: "13px 15px", fontSize: "13px", color: th.textSub, display: "flex", alignItems: "center", gap: "8px", marginBottom: "14px", textAlign: "left", cursor: "pointer", fontFamily: F, fontWeight: 500 }}>
      <span style={{ fontSize: "16px", color: th.accent }}>+</span> Přidat úkol...
    </button>
  );
  ...
  return (
    <div style={{ ...cardS(th), padding: "14px", marginBottom: "14px", animation: "sUp 0.2s" }}>
      <div style={{ display: "flex", gap: "4px", marginBottom: "10px" }}>
        {[{ id: "simple", l: "Jednoduchý úkol" }, { id: "complex", l: "Komplexní (s checklistem)" }].map(tp => (
          <button key={tp.id} onClick={() => setType(tp.id)} style={{ ...btnS(th), flex: 1, padding: "8px", fontSize: "11px", background: type === tp.id ? th.accentSoft : "transparent", color: type === tp.id ? th.accent : th.textSub, border: `1px solid ${type === tp.id ? th.accentBorder : th.cardBorder}` }}>{tp.l}</button>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px", marginBottom: "10px" }}>
        <div><div style={lbl}>Pro koho</div>
          ...
          {assign === "person" && others.length > 1 && (
            <select value={assignTarget} onChange={e => setAssignTarget(e.target.value)} style={{ ...inpS(th), marginTop: "4px" }}>
        ...
        <div><div style={lbl}>Termín</div>
          <input type="date" value={due} onChange={e => setDue(e.target.value)} style={inpS(th)} /></div>
      </div>
      <button onClick={() => setAdv(!adv)} style={{ background: "none", border: "none", color: th.textMid, fontSize: "11px", cursor: "pointer", fontFamily: F, fontWeight: 600, padding: "4px 0", marginBottom: adv ? "8px" : "10px" }}>
        {adv ? "▲ Méně" : "▼ Rozšířené nastavení"}
      </button>
      {adv && (
        <div style={{ marginBottom: "10px", animation: "fIn 0.12s" }}>
          <div><div style={lbl}>Opakování</div>
            <select value={rec} onChange={e => setRec(Number(e.target.value))} style={inpS(th)}>
      ...
      <div style={{ display: "flex", gap: "8px" }}>
        <button onClick={submit} style={{ ...btnS(th), flex: 1, padding: "11px", background: th.accent, color: "#fff", fontSize: "14px" }}>Přidat úkol</button>
        <button onClick={() => setOp(false)} style={{ ...btnS(th), padding: "11px 16px", background: "transparent", color: th.textSub, border: `1px solid ${th.cardBorder}`, fontSize: "14px" }}>Zrušit</button>
      </div>
    </div>
  );
}
...
function Snack({ msg, onUndo, vis, th }) {
  if (!vis) return null;
  return (
    <div style={{ position: "fixed", bottom: "24px", left: "50%", transform: "translateX(-50%)", background: th.snackBg, border: `1px solid ${th.cardBorder}`, borderRadius: "12px", padding: "12px 16px", display: "flex", alignItems: "center", gap: "12px", zIndex: 100, animation: "snIn 0.25s ease", fontFamily: F, boxShadow: "0 8px 32px rgba(0,0,0,0.4)", maxWidth: "90vw" }}>
...
export default function App() {
  ...
  useEffect(() => { if (user) localStorage.setItem("ft_user", JSON.stringify(user)); else localStorage.removeItem("ft_user"); }, [user]);
  ...
  useEffect(() => {
    (async () => {
      ...
      setUsers(u);
      const { tasks: p, updates } = processRec(t);
      setTasks(p);
  ...
  useEffect(() => {
    if (loading) return;
    const tCh = supabase.channel("tasks-changes")
      .on("postgres_changes", { event: "*", schema: "public", table: "tasks" }, payload => {
        if (payload.eventType === "INSERT") {
          setTasks(p => p.find(t => t.id === payload.new.id) ? p : [dbToTask(payload.new), ...p]);
        } else if (payload.eventType === "UPDATE") {
          setTasks(p => p.map(t => t.id === payload.new.id ? dbToTask(payload.new) : t));
        } else if (payload.eventType === "DELETE") {
          setTasks(p => p.filter(t => t.id !== payload.old.id));
        }
      }).subscribe();
    const uCh = supabase.channel("users-changes")
  ...
  // Recurring check + keep-alive ping (prevents Supabase pause)
  useEffect(() => {
    const iv = setInterval(() => {
      setTasks(p => {
        const { tasks: u2, updates } = processRec(p);
        if (updates.length > 0) apiUpdateTasks(updates);
        return updates.length > 0 ? u2 : p;
      });
      // Daily ping to keep Supabase project active
  ...
  const removeUser = useCallback(async (name) => {
    await apiDeleteUser(name);
    const updated = tasks.map(t => ({
      ...t,
      assignedTo: t.assignedTo?.filter(n => n !== name),
      doneBy: t.doneBy?.filter(n => n !== name),
      seenBy: t.seenBy?.filter(n => n !== name),
    }));
    apiUpdateTasks(updated);
  }, [tasks]);

  const doUndo = useCallback(async () => {
    if (!undo) return;
    setTasks(undo.prev);
    apiUpdateTask(undo.prev.find(t => t.id === undo.taskId));
  ...
  const addTask = useCallback(async (t) => {
    ...
    if (t.assignTo !== "self") noti(`📋 Nový úkol od ${t.createdBy}`, t.title);
  }, []);
  ...
  const chgStatus = useCallback((id, action) => {
    ...
    withUndo(m, id, prev => prev.map(t => {
      ...
      if (action === "done_my") {
        const nb = [...new Set([...(t.doneBy || []), user.name])];
        const all = users.every(u => nb.includes(u.name));
        return { ...t, doneBy: nb, status: all ? "done" : t.status, completedAt: all ? now : t.completedAt, completedByUser: all ? user.name : t.completedByUser };
      }
  ...
  const unread = useMemo(() => {
    ...
    const r = {};
    users.forEach(u => { r[u.name] = tasks.filter(t => !t.seenBy?.includes(u.name) && t.createdBy !== u.name && !dn(t)).length; });
    return r;
  }, [tasks, user, users]);
  ...
  const stats = useMemo(() => {
    ...
    return {
      my: a.filter(t => t.assignedTo?.includes(user.name)).length,
      out: a.filter(t => t.createdBy === user.name && !t.assignedTo?.every(x => x === user.name)).length,
      shared: a.filter(t => t.assignTo === "both").length,
    };
  }, [tasks, user]);
    ...
    <div style={{ minHeight: "100vh", background: th.bg, fontFamily: F, color: th.text, WebkitFontSmoothing: "antialiased" }}>
      ...
      <div style={{ background: th.headerBg, backdropFilter: "blur(20px)", borderBottom: `1px solid ${th.cardBorder}`, padding: "11px 14px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 30 }}>
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          <button onClick={toggleTheme} style={{ background: "none", border: "none", cursor: "pointer", fontSize: "14px", padding: "2px" }}>
            {theme === "dark" ? "☀️" : "🌙"}
          </button>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
          {users.filter(u => u.name !== user.name).map(u => unread[u.name] > 0 && (
            <span key={u.name} style={{ fontSize: "10px", color: th.textSub }}>{u.name}: <span style={{ background: th.yellow, color: "#fff", borderRadius: "8px", padding: "1px 5px", fontSize: "9px", fontWeight: 800 }}>{unread[u.name]}</span></span>
          ))}
          {user.admin && <button onClick={() => setShowAdmin(!showAdmin)} style={{ background: "none", border: "none", color: th.textSub, cursor: "pointer", fontSize: "12px", fontFamily: F }}>⚙️</button>}
          <button onClick={() => setUser(null)} style={{ ...btnS(th), background: th.inputBg, border: `1px solid ${th.inputBorder}`, color: th.textSub, padding: "5px 10px", fontSize: "11px", display: "flex", alignItems: "center", gap: "4px" }}>
            {user.name}
          </button>
        </div>
      </div>
      <div style={{ maxWidth: "560px", margin: "0 auto", padding: "14px 12px 140px" }}>
        ...
        <div style={{ display: "flex", gap: "3px", marginBottom: "6px", overflowX: "auto", paddingBottom: "2px" }}>
          {[
            { id: "my", l: "Moje", n: stats.my },
            { id: "assigned", l: "Zadané", n: stats.out },
            { id: "shared", l: "Společné", n: stats.shared },
            { id: "unread", l: "Nepřečtené", n: unread[user.name] },
            { id: "all", l: "Vše" },
          ].map(tab => (
            <button key={tab.id} onClick={() => setFilter(tab.id)} style={{ ...btnS(th), padding: "6px 10px", fontSize: "11px", background: filter === tab.id ? th.accentSoft : "transparent", color: filter === tab.id ? th.accent : th.textMid, border: filter === tab.id ? `1px solid ${th.accentBorder}` : "1px solid transparent", display: "flex", alignItems: "center", gap: "4px", whiteSpace: "nowrap" }}>
              {tab.l}
              {tab.n > 0 && <span style={{ background: filter === tab.id ? th.accent : th.btnBg, color: filter === tab.id ? "#fff" : th.textSub, borderRadius: "8px", padding: "0 5px", fontSize: "9px", fontWeight: 800 }}>{tab.n}</span>}
            </button>
          ))}
        </div>
        ...
        <div style={{ display: "flex", gap: "3px", marginBottom: "12px", overflowX: "auto", paddingBottom: "2px" }}>
          ...
          {CATS.map(c => {
            const cnt = tasks.filter(t => t.category === c.id && (vs === "active" ? !dn(t) : vs === "done" ? t.status === "done" : t.status === "cancelled")).length;
            if (cnt === 0 && catF !== c.id) return null;
            return <button key={c.id} onClick={() => setCatF(c.id)} style={{ ...btnS(th), padding: "3px 7px", fontSize: "10px", background: catF === c.id ? th.btnBg : "transparent", color: catF === c.id ? th.text : th.textDim, whiteSpace: "nowrap" }}>{c.icon} {cnt}</button>;
          })}
        </div>
        {filtered.length === 0 ? (
          <div style={{ textAlign: "center", color: th.textDim, padding: "50px 20px" }}>
            ...
            <div style={{ fontSize: "13px", fontWeight: 500, color: th.textMid }}>
              {vs === "active" ? "Žádné aktivní úkoly" : vs === "done" ? "Zatím nic splněného" : "Žádné nerealizované"}
            </div>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: "5px" }}>
            {filtered.map(t => (
              <TaskCard key={t.id} task={t} user={user} users={users} onStatus={chgStatus} onSeen={markSeen} onUpdate={updateTask} th={th} />
            ))}
          </div>
        )}
      </div>
